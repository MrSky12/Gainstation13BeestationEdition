	...
	if(href_list["newspost"])
		...
		for(var/datum/feed_channel/FC in GLOB.news_network.network_channels)
			if(FC.channel_name == "GATO Book Club")
				...
				break
		if(!channelexists)
			GLOB.news_network.create_feed_channel("GATO Book Club", "Library", null)
		GLOB.news_network.submit_article(scanner.cache.dat, "[scanner.cache.name]", "GATO Book Club", null)
